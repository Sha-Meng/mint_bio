...
## Product Overview
荣誉墙位于企业概况页，PC 组件 `CorporateVision` 与移动端组件 `CorporateVisionMobile` 都通过 `getText('corporate.honors')` 读取数组，并用 `HonorCard` 渲染 `line1` / `line2`。
...
## Key Code Structures
目标 Directus keys 为 `corporate.honors.0.line1` 到 `corporate.honors.11.line2`，共 24 个 leaf entries。
