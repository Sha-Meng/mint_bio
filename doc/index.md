...
- 官网前端：Vue 3 + Vue CLI + Vue Router + Element Plus。
- 页面架构：PC / Mobile 双版本组件，通过屏幕宽度分流。
- 新闻模块：使用 Directus 作为当前内容数据源。
...
| [正文中英文对照表配置](./operations/directus-i18n-guide.md) | 面向运营和内容负责人，说明官网固定文案、新闻英文内容、英文入口和翻译来源要求。 |
...
| [账号凭据查询说明](./ops-dev/credentials-handoff-template.md) | 说明服务器、宝塔、Directus、MySQL、阿里云等凭据应联系开发在哪里查询。**不保存真实密码**。 |
...
本次交接截图统一放在 `assets/handoff/`，用于辅助定位阿里云、宝塔、Directus 新闻、固定文案和数据模型界面。截图只作为界面位置参考，真实配置以当前控制台和服务器状态为准。

## 当前维护边界

- 新闻、图片和正文内容在 Directus 中维护。
- 日常固定文案维护在 Directus `site_i18n_entries` 中完成；`src/i18n/*.json` 仅作为前端紧急 fallback。
- 运营不得自行根据上下文翻译英文内容；翻译必须来自历史中英对照资料或用户确认稿。
- 仓库文档不得保存真实服务器密码、宝塔密码、数据库密码、Token、私钥；本地私密交接资料可放在已忽略的 `doc/private/`。
