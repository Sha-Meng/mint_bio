...
function desiredEntries() {
  return features.map((feature, index) => ({
    key_path: `newMaterial.features.${index}`,
    group: "newMaterial",
    label: feature,
    value_zh: feature,
    value_en: "",
...
async function fetchExistingEntries(env) {
  ...
  qs.set("filter[key_path][_starts_with]", "newMaterial.features.");
...
function buildPlan(existing) {
  ...
  existing.forEach((entry) => {
    ...
    if (!/^newMaterial\.features\.\d+$/.test(entry.key_path)) return;
...
async function main() {
  ...
  if (APPLY) {
    ...
    if (changed) console.log("Applied new material features update.");
  }
}
